...
const Register = () => {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      ...
      toast(error?.response?.data?.message ?? "Could not register", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-white to-[#F7FBFF] overflow-hidden relative">
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-8 py-12 min-h-screen flex items-center relative z-10">
        {/* Left: Form */}
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }} className="flex-1 max-w-md mx-auto w-full">
          <motion.div
          ...
          >
            ...
            <div className="text-center mb-8">
              ...
              >
                Start your secure health records journey
              </motion.p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              >
                ...
                <div className="relative">
                  ...
                  <input type="text" name="name" value={form.name} onChange={handleChange} required className="w-full pl-12 pr-4 py-3 rounded-xl border border-gray-200 bg-white/50 backdrop-blur-sm text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-[#0C6CF2] focus:border-transparent transition-all" placeholder="John Doe" />
                </div>
              </motion.div>
              ...
              >
                ...
                <div className="relative">
                  ...
                  <input type="email" name="email" value={form.email} onChange={handleChange} required className="w-full pl-12 pr-4 py-3 rounded-xl border border-gray-200 bg-white/50 backdrop-blur-sm text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-[#0C6CF2] focus:border-transparent transition-all" placeholder="your.email@example.com" />
                </div>
              </motion.div>
              ...
              >
                ...
                <div className="relative">
                  ...
                  <input type="text" name="tenantName" value={form.tenantName} onChange={handleChange} required className="w-full pl-12 pr-4 py-3 rounded-xl border border-gray-200 bg-white/50 backdrop-blur-sm text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-[#0C6CF2] focus:border-transparent transition-all" placeholder="Your Organization" />
                </div>
              </motion.div>
              ...
              >
                ...
                <div className="relative">
                  ...
                  <input type={showPassword ? "text" : "password"} name="password" value={form.password} onChange={handleChange} required minLength={8} className="w-full pl-12 pr-12 py-3 rounded-xl border border-gray-200 bg-white/50 backdrop-blur-sm text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-[#0C6CF2] focus:border-transparent transition-all" placeholder="Minimum 8 characters" />
            ...
            >
              Already have an account? <Link to="/login" className="font-semibold text-[#0C6CF2] hover:text-[#0A5CD9] transition-colors">Sign in</Link>
            </motion.p>
          </motion.div>
        </motion.div>

        {/* Right: Medical Illustration */}
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8, delay: 0.3 }} className="hidden lg:flex flex-1 items-center justify-center relative">
          <div className="relative w-full max-w-lg">
            <div className="absolute inset-0 bg-gradient-to-br from-[#0C6CF2]/20 to-[#00A1A9]/20 rounded-3xl blur-3xl -z-10" />
            <motion.img src="/scansehat-hero.png" alt="Medical illustration" initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ duration: 0.8, delay: 0.5 }} className="w-full h-auto rounded-2xl shadow-2xl" onError={(e) => { console.warn("IMAGE MISSING: /scansehat-hero.png"); (e.target as HTMLImageElement).style.display = "none"; }} />
          </div>
        </motion.div>
      </div>
    </div>
